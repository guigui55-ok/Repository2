...
namespace ExcelCellsManager
{
    public partial class ExcelCellsManagerForm : Form
    {
        ...
        protected ExcelWorkbookList.ExcelWorkbookList _workbookList;
        protected ExcelWorkbookList.WorkbookListCheckedListBox _workbookListCheckedListBox;
        protected ExcelWorkbookList.IWorkbookListControl _workbookListControl;
        // Contorl Relation
        ...
        public ExcelCellsManagerForm()
        {
            try
            {
                ...
                _excelCellsManagerMain = new ExcelCellsManagerMain(_error, this)
                {
                    ...
                    OpenedFile = new OpenedFile(_error),
                ...
                if(_excelCellsManagerMain.ExcelManager.GetExcelAppsList().Count < 1)
                {
                    ...
                    _excelCellsManagerMain.ExcelManager.ExcelApplicationRunWhenNothing(true);
                    if (_error.HasAboveWorning()) { _error.AddLogAlert("UpdateOpendExcelApplication Failed"); }
                }
                // 一つ以上の Workbook が開かれている
                if (!_excelCellsManagerMain.ExcelManager.AppsListIsValid())
                {
                    // ActiveCell を取得・設定する
