...
namespace ExcelCellsManager
{
    public class ExcelCellsManagerMain
    {
        ...
        public ExcelWorkbookList.ExcelWorkbookList ExcelWorkbookList;
        ...
        public void SetStatusBarText(string msg)
        {
            ...
            {
                _error.AddException(ex,this, "SetStatusBarText");
            }
        }
        ...
        // ExcelApps を ExcelCellsInfo2 より取得する。Workbook を開いていない場合は開く。
        // 取得した際に AppsInfo にデータを格納する。Workbook を開いた場合は isOpendExcel=true にする
        public ExcelApps GetAppsWithOpenWorkbook(ref ExcelCellsInfo2 cellsInfo,ref AppsInfo appsInfo,ref bool isOpendExcelFile)
        {
            try
            {
                ...
                if (apps == null)
                {
                    ...
                    // Index をセットする
                    //appsInfo.Index = WorkbookListControl.GetIndexForExcelAppsFromCheckedListBox(cellsInfo.BookName);
                    if (_error.hasAlert)
                    {
        ...
        public ExcelApps GetExcelAppsSelectedRowInDataGridViewAfterOpenWorkbookWhenNotOpened(ExcelCellsInfo2 cellsInfo)
        {
            try
            {
                ...
                AppsInfo appsInfo = new AppsInfo
                {
                    ProcessId = 0,
                    FileName = cellsInfo.BookName,
                    Index = GetIndexIncludeBookNameInCheckedListBox(cellsInfo.BookName)
                };
                ...
                if ((appsInfo.ProcessId <= 0)||(appsInfo.Index < 0))
                {
                    // WorkbookList にはない Workbook → 開いていない →開く
                    _error.AddLog("Workbook Not Opend.  * Workbook.ProcessID = 0 [" + appsInfo.FileName + "]");
                    ExcelManager.OpenFile(cellsInfo.Path + "\\" + cellsInfo.BookName);
        ...
        private int GetIndexIncludeBookNameInCheckedListBox(string bookname)
        {
            try
            {
                _error.AddLog(this, "GetIndexIncludeBookNameInCheckedListBox");
                List<string> list = WorkbookListControl.GetValueList();
                if(list.Count < 1) { _error.AddLogWarning(" WorkbookListControl.GetValueList.Count < 1"); return -2; }
                int count = 0;
                ...
                _error.AddLogWarning(" BookName is Nothing. bookname="+bookname);
                return -3;
            } catch (Exception ex)
            {
                _error.AddException(ex, this, "GetIndexIncludeBookNameInCheckedListBox");
                return -1;
            }
        }
        ...
        public void ActivateWorkbookSelectedCheckedListBox()
        {
            try
            {
                ...
                string[] item = WorkbookListControl.GetSelectedItems();
        ...
        private List<AppsInfo> GetAppsInfoListFromCheckdItemList()
        {
            List<AppsInfo> ret = new List<AppsInfo>();
            try
            {
                _error.AddLog(this, "GetAppsInfoListFromCheckdItemList");
                string[] items = WorkbookListControl.GetSelectedItems();
                if(items.Length < 1) { _error.AddLog(" items.Length < 1"); return ret; }
                AppsInfo info;
                foreach(string item in items)
                {
                    info = WorkbookListControl.ConvertToAppsInfoFromItemValue(item);
                    ret.Add(info);
                }
                return ret;
            } catch (Exception ex)
            {
                _error.AddException(ex, this, "GetAppsInfoListFromCheckdItemList");
                return ret;
            }
        }
