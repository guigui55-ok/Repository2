...
namespace Utility
{
    ...
    public static class Handles
    {
        ...
        static readonly int TimeOut = 50;
            ...
            using (var proc = Process.GetProcessById(pid))
            {
                ...
                foreach (var shi in EnumHandles(pid))
                {
                    ...
                    try
                    {
                        ...
                        {
                            Debug.Print("Debug [CloseHandleUtil:Line 112]");
                        }
                            ...
                            else
                            {
                                using (var nto2 = new NtObject(hObj, ObjectInformationClass.ObjectNameInformation, typeof(OBJECT_NAME_INFORMATION)))
                                {
                                    try
                                    {
                                        ...
                                        {
                                            //.WriteLine("nto2 is null");
                                        }

                                    }
                                    catch (Exception e)
                                    {
                                        Debug.Print("nto2.Buffer = " + nto2.Buffer);
                                        Debug.Print(e.Message);
                                        Debug.Print(e.Source);
                                        Debug.Print(e.StackTrace);
                                    }
        ...
        static OBJECT_TYPE_INFORMATION ObjectTypeInformationFromBuffer(IntPtr buffer)
        {
            unsafe
            {
                if (buffer == (IntPtr)0)
                {
                    return new OBJECT_TYPE_INFORMATION();
                }
                else
                {
        ...
        static OBJECT_NAME_INFORMATION ObjectNameInformationFromBuffer(IntPtr buffer)
        {
            unsafe
            {
                if (buffer == (IntPtr)0)
                {
                    return new OBJECT_NAME_INFORMATION();
                } else {
        ...
        class NtObject : IDisposable
        {
            ...
            public void Init(IntPtr hObj, ObjectInformationClass infoClass, int estimatedSize)
            {
                ...
                Buffer = QueryWithTimeout(hObj, infoClass, estimatedSize, TimeOut);
            }
            ...
            public static IntPtr QueryWithTimeout(IntPtr hObj, ObjectInformationClass infoClass, int estimatedSize, int timeout)
            {
                IntPtr QueryRet = (IntPtr)0;
                try
                {
                    //Thread t = new Thread(Query(hObj, infoClass, estimatedSize));
                    ...
                    Task ret = Task.Run(() =>
                    {
                        try
                        {
                            QueryRet = Query(hObj, infoClass, estimatedSize);
                    ...
                    {
                        // タスクをキャンセルする
                        Console.WriteLine("Query Cancel");
                        Console.WriteLine("IntPtr hObj =" + hObj);
                        Console.WriteLine("ObjectInformationClass infoClass = " + infoClass.ToString());
                        //Console.WriteLine((int)infoClass);
                        Console.WriteLine("estimatedSize = "+ estimatedSize);
                        tokenSource.Cancel();
                        return (IntPtr)0;
                    }
                    //if (ret.GetAwaiter())
                ...
                {
                    Console.WriteLine("HandleInfo.QueryWithTimeout");
            ...
            //async Task<IntPtr> QueryAsynca(IntPtr hObj, ObjectInformationClass infoClass, int estimatedSize)
            //{
            ...
            //    {
            //        //ここで止まることがある
            //       NT_STATUS ret = NtQueryObject(hObj, infoClass, buf, size, out retsize);
            //        if (NT_SUCCESS(ret))
            //        {
            //            return buf;
            //        }
